...
\begin{frame}{Outline}
	...
	I will start by describing how to associate certain topological invariants to data sets, I'll illustrate their use with a real world example, and introduce one of the computational tools available for their incorporation into machine learning and data exploration.
...
\begin{frame}{Point clouds}
	\note{\small
		...
		\vk		
		Let me tell you about a method we can use to approximate it.
		Given a scale parameter $r$, we can construct a triangulation as follows.		
		...
		Here we see 4 examples of triangulations constructed with increasing values of the parameter.
		\vk
		...
		\vk
		We will exploit this coherence after we overview a topological invariant we can construct for any triangulated space.
...
\begin{frame}[c]{Homology}
	...
	\begin{center}
		...
		\qquad\qquad
		\pause
		\includegraphics[scale=.8]{simplex}
	\end{center}
...
\begin{frame}{Persistence homology}
	...
	\begin{center}
		\includegraphics[scale=.8]{diagram.pdf}
	\end{center}
\end{frame}
...
\begin{frame}{Example: nanoporous materials}
	...
	There are a lot of such materials, and many more are being made rapidly these days.
	...
	\begin{center}
		\includegraphics[scale=.085]{hess}
	\end{center}
...
\begin{frame}{Example: nanoporous materials}
	...
	\note{There is a catalogue of over 3 million nanoporous structures. One would like to compare the geometries of these, but direct inspection at this scale is out of the question.
	\vk
	The authors of this paper constructed persistence diagrams using dimensions 0, 1 and 2 for each structure. They used the methods we discussed before starting with a point cloud of equidistantly distributed points in the cavities of a fundamental domain.
	\vk
	...
	\vk
	The authors of this paper, were able to identify in this way materials with very similar nanoporous structures missed by other methods. Here are some examples. In each row there are distinct nanoporous materials that were identified to be have very similar geometries.
	\vk
	...
	\begin{textblock*}{4cm}(5.4cm,1.5cm)
		\includegraphics[scale=.28]{real_material}
	\end{textblock*}
	...
	\vspace*{-1cm}
	\hspace*{3.8cm}\includegraphics[scale=.6]{nanoporous}
\end{frame}
...
\begin{frame}{Machine Learning and Topology}
	...
	\vk
	A Python package for TDA built on top of high performance C++ implementations, that closely follows the operational structure of scikit-learn, and integrates effortlessly into machine learning pipelines.
	\vk
	...
	\includegraphics[scale=.33]{giotto}	
\end{frame}
...
\begin{frame}{Finer topological invariants}
	...
	I will define these in the next slides, emphasising how they are both induced from a structure at the cochain level.
	
	Before going there thous, a quick comment for the experts, in case you are wondering about coefficients here, notice that we can replace the torus and the Klein bottle by $\mathbb CP^2$ and the wedge of a 4 and a 2 sphere to make the same point over the integers.}
...
\begin{frame}[fragile]{Alexander-Whitney \& Steenrod}
	...
	Although the resulting cup product in cohomology is graded commutative, the cup-0 product of cochains it's not. We can see that by just transposing the factors in this tensor product.
	...
	cup-1, satisfying that the coboundary of alpha cup1 beta is equal to alpha cup0 beta - beta cup0 alpha. So cup0 is commutative up to an explicit coboundary given by the cup-1.
	\vk
	We notice some symmetry here, because, after permuting alpha and beta, we see that the coboundary of beta cup1 alpha is also equal to the right hand side. Another coboundary witnessing the commutativity of cup-$0$.}
...
\begin{frame}[fragile, noframenumbering]{Alexander-Whitney \& Steenrod}
	...
	\note{In general cup-1 is itself not commutative. So Steenrod constructed an explicit cup-2 product correct for the lack of commutativity of cup-1.
	}
...
\begin{frame}[fragile, noframenumbering]{Alexander-Whitney \& Steenrod}
	...
	\note{We can keep repeating this and get the cup-$i$ products of Steenrod, a coherent collection of products whose coboundaries correct the lack of commutativity of the previous correction.
	...
	$\delta(\alpha \smallsmile_i \beta) = \alpha \smallsmile_{i-1} \beta\, \pm\, \beta \smallsmile_{i-1} \alpha$
...
\begin{frame}{Secondary operations}
	\note{Steenrod squares, invariants at the cohomology level, are defined by the cup-$i$ products which arise from lifting to the cochain level the commutativity relation of the cup product in cohomology. Recall, that the coboundary of cup-1 is the difference between cup-0 and its transpose.
	
	We are motivated then to look for other cohomological relations to lift. Steenrod squares, examples of primary cohomology operations, satisfy relations themselves and the invariants that appear by lifting and solving these relations are known as secondary operations. \press
	...
	Can we lift these the cochain level? \press
	...
	So we wonder if we can solve these equation, effectively constructing such coboundaries, and unlock secondary cohomology operations
}
...
\begin{frame}{Graph Laplacian}
	...
	Where signal in this context means a real-valued function from the set of vertices of the graph. \press
...
\begin{frame}{Information signals}
	...
	For us is not so important to go over these definitions. It suffices to know they exist, and, therefore, that natural signals properly defined over simplices of higher dimensions exist.
	
	For example, to edges we can assign the mutual information shared by their vertices, for triangles, the interaction information, and to higher dimensional simplices, we can assign appropriate generalizations of these. \press
	...
	\begin{center}
		\includegraphics[scale=.12]{fishes}
	\end{center}
...
\begin{frame}{Hyperharmonic analysis}
	...
	\note{Here is an analogy. The speakers in our cellphones are to small to produce the fundamental vibration of half of the people's voices. But we still recognize them just hearing a lower fidelity version, slash lower dimensional representation of their voices. \press
		
	As another example, we notice that just a few harmonics are needed to distinguish a piano from a guitar playing the same note.
	
	Can we do something like that for information signals?
	
	Yes, indeed. \press
	
	Together with these researchers, we proposed a method that allows us to compress these signals substantially, reducing in this way some of the issued associated to the curse of dimensionality. \press
	...
	Consider a signal and a fix basis. Without lose of generality, order the coefficients of this signal in that basis so that their squares are non-increasing.
	The $k$-th explained variance is the percentage of the variance explained by the $k$-th coefficient, whereas the $k$-th cummulative explained variance is that explained by the first $k$ coefficients.
}
...
\begin{frame}{Proof of concept: Hadyn's symphonies}
	...
	We computed them in dimension 2, 3, 4 and 5, corresponding to subsets of nodes of cardinalities 3, 4, 5 and 6.
...
{
	\usebackgroundtemplate{\includegraphics[width=\paperwidth]{castillo.jpg}}%
	\begin{frame}
		\vskip 6cm
		\begin{center}
